class Reports::ConsumableItemsController < ApplicationController
	
   def index
   	session[:report_items] = {}
   	
  	@items = Item.find(:all, :conditions => {:type_of => Item::CONSUMEABLETYPE})
  	_init
  ...
  def export_list
  	...
  	type_name = Item::CONSUMEABLETYPE
